/*
 * Rounded-rectangle refraction shader adapted from BiliPai.
...
fun BackdropEffectScope.liquidLens(
    ...
    val shader = if (dispersionEnabled) {
    ...
    val scaledHeight = size.height / scaleFactor
    val scaledPadding = padding / scaleFactor
    val scaledRadii = FloatArray(radii.size) { radii[it] / scaleFactor }
    runtimeShaderEffect(
        key = if (dispersionEnabled) "MarkstashLiquidLensDispersion" else "MarkstashLiquidLens",
        shaderString = shader,
        uniformShaderName = "content",
    ) {
        setFloatUniform("size", scaledWidth, scaledHeight)
        setFloatUniform("offset", -scaledPadding, -scaledPadding)
        setFloatUniform("cornerRadii", scaledRadii)
        setFloatUniform("refractionHeight", refractionHeight / scaleFactor)
        setFloatUniform("refractionAmount", -refractionAmount / scaleFactor)
...
private fun BackdropEffectScope.roundedRectCornerRadii(): FloatArray? {
    ...
    val topLeft = if (isLtr) {
        cornerShape.topStart.toPx(size, this)
    } else {
        cornerShape.topEnd.toPx(size, this)
    }
    ...
    val bottomRight = if (isLtr) {
        cornerShape.bottomEnd.toPx(size, this)
    } else {
        cornerShape.bottomStart.toPx(size, this)
    }
